...
本目录是第四问的独立实现，采用“全局三角网格保证发现—保守位置域—有限局部回退—光学覆盖清除”路线。它复用 `questions.q3.shared` 中的协议数据结构思想，但不直接复制第三问策略。
...
```powershell
python -m questions.q4.run --backend local --seed 0 --output outputs/questions/q4/demo.json
python -m questions.q4.backtest --seeds 0:4 --output outputs/questions/q4/baseline.csv
python -m questions.q4.backtest --seeds 0:19 --global-route nearest --output outputs/questions/q4/nearest_after_opt.csv
```
